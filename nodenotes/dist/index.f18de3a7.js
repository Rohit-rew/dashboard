...
async function renderNotes() {
    try {
        ...
        let html = notes.data.data.map((note)=>{
            return `
        <div class="notes"> 
        <input class="checkbox" type="checkbox" ${note.checked ? "checked" : ""} data-id="${note._id}"/>
        <p style="text-decoration : ${note.checked ? "line-through" : ""}">${note.text} </p>
...
document.addEventListener("click", async (e)=>{
    if (e.target && e.target.matches(".deletebtn")) {
        ...
        await fetch(`http://localhost:4500/api/notes/${id}`, {
            method: "DELETE"
        });
        renderNotes();
    } else if (e.target && e.target.matches(".checkbox")) {
        let id1 = e.target.dataset.id;
        await axios.put(`http://localhost:4500/api/notes/${id1}`, {
...
},{}]},["37eO2","7SwCM"], "7SwCM", "parcelRequirefe55")
